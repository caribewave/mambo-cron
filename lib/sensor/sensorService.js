...
const edit = async (label, sensorParam) => {

  let sensor = Sensor.find({
    label: label
  });


  return await Sensor.update({label: label}, {
    $set: {
      label: sensorParam.label || sensor.label,
      activated: sensorParam.activated || sensor.activated,
      source: sensorParam.source || sensor.source,
      type: sensorParam.type || sensor.type
    }
  }, {new: true});
};
